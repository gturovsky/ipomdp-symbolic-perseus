...
public class DDnode extends DD {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5396834073537667586L;
	...
	public static DD myNew(int var, DD[] children) {
		...
		WeakReference<Object> storedNode = (WeakReference<Object>) Global.nodeHashtable.get(node);
	...
	public int[] getVarSet() {
		if (varSet == null) {
			...
			for (int childId = 0; childId < children.length; childId++) {
				varSet = MySet.unionOrdered(children[childId].getVarSet(),
						varSet);
			}
		}
		return varSet;
	...
	public double getSum() {
		if (sum == Double.NaN) {
			...
			for (int i = 0; i < children.length; i++) {
				int[] remainingVars = MySet.diff(childrenVars,
						children[i].getVarSet());
	...
	public void display(String space) {
		if (Global.varNames == null) {
			System.out.println(space + "var: " + Integer.toString(var) + "  "
					+ MySet.toString(varSet));
		} else {
			System.out.println(space + Global.varNames[var - 1] + "  "
					+ MySet.toString(varSet));
		}
	...
	public void display(String space, String prefix) {
		if (Global.varNames == null) {
			System.out.println(space + prefix + "var: " + Integer.toString(var)
					+ "  " + MySet.toString(varSet));
		} else {
			System.out.println(space + prefix + Global.varNames[var - 1] + "  "
					+ MySet.toString(varSet));
		}
	...
	public void printDotDD(PrintStream ps, String name) {
		...
		ps.println("{rank = same; node [shape=ellipse, style=filled, color=cornflowerblue];\""
				+ name + "\" [label=\"" + Global.varNames[var - 1] + "\"];}");
		String newname;
		for (int i = 0; i < children.length; i++) {
			newname = name + "c" + i;
			children[i].printDotDD(ps, newname);
			ps.println("\"" + name + "\" -> \"" + newname + "\" [label = \""
					+ Global.valNames[var - 1][i] + "\"];");
		}
	}
}