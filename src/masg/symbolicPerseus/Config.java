...
public class Config {
		...
		if (config1 == config2)
			...
			if (config1[0].length == config2[0].length) {
				for (int i1 = 0; i1 < config1[0].length; i1++) {
					...
					if (i2 == -1)
						return false;
					else if (config1[1][i1] != config2[1][i2])
						return false;
				}
				return true;
			} else
	...
	public static int[][] merge(int[][] config1, int[][] config2) {
		...
		if (config1 == null)
			return Config.clone(config2);
		if (config2 == null)
			return Config.clone(config1);
	...
	public static int[][] add(int[][] config, int var, int val) {
		...
		if (index == -1) {
			...
			newConfig[1][index] = val; // should double check that values are
										// the same
		}
	...
	public static int[][] extend(int[][] config, int[] vars) {
		...
		if (config == null)
			...
			missingVars = MySet.diff(vars, config[0]);
	...
	public static int[][] intersection(int[][] config, int[] vars) {
		...
		if (config == null || config[0].length == 0 || vars == null
				|| vars.length == 0)
			return new int[2][0];
