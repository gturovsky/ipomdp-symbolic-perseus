package masg.symbolicPerseus.spudd;
...
import masg.symbolicPerseus.dd.DDnode;
...
public class ParseSPUDD {
	...
	private void error(String errorMessage) {
		System.out.println("Parse error at " + stream.toString() + ": "
				+ errorMessage);
		// System.exit(1);
	}
		...
		try {
			...
			while (true) {
				...
				case StreamTokenizer.TT_EOF:
					...
					Global.setValNames(Global.valNames.length + 1,
							actNamesArray);
	...
	public void createPrimeVars() {
		...
		for (int i = 0; i < varNames.size(); i++)
			varNamesArray[i] = varNames.get(i);
		...
		for (int varId = 0; varId < Global.varNames.length / 2; varId++) {
			...
			DD dd = DDnode.myNew(varId + 1 + Global.varNames.length / 2,
					children);
		...
		for (int varId = 0; varId < Global.varNames.length / 2; varId++) {
			for (int valId = 0; valId < Global.varDomSize[varId]; valId++) {
				String ddName = Global.varNames[varId]
						+ Global.valNames[varId][valId];
				DD[] children = new DD[Global.varDomSize[varId]];
				for (int i = 0; i < Global.varDomSize[varId]; i++) {
					if (valId == i)
						children[i] = DD.one;
		...
		try {
			...
			// arithmetic operation
				...
				// threshold operator - why was this developed?
					...
					DD decreaseFactors = OP.threshold(newDd, 1, -1); // is newDd
																		// wherever
																		// newDd
																		// <= 1,
																		// is 0
																		// elsewhere
					DD increaseFactors = OP.threshold(newDd, 1, 1); // is newDd
																	// wherever
																	// newDd >=
																	// 1, is 0
																	// elsewhere
					DD staysameFactors = OP.threshold(newDd, 1, 0); // is newDd
																	// wherever
																	// newDd ==
																	// 1, is 0
																	// elsewhere
		...
		try {
			...
			while (true) {
				...
				// observation function
					...
					while (true) {
						...
						// obs cpt
							...
							DD normalizationFactor = OP.addMultVarElim(dds,
									vars);
							if (unnormalized)
								obsCPTs[varId - nStateVars] = OP.div(
										obsCPTs[varId - nStateVars],
										normalizationFactor);
							else if (OP.maxAll(OP.abs(OP.sub(
									normalizationFactor, DD.one))) > 1e-8)
								error("Unnormalized cpt for "
										+ varNames.get(varId) + "'");
						}
					}
				...
				// cpt
					...
					else if (OP.maxAll(OP.abs(OP.sub(normalizationFactor,
							DD.one))) > 1e-8)
