...
class OP {
	...
	// ////////////////////////////////////////////////////
	// shift variable index by of all variables by n (useful when priming
	// variables in value iteration)
	// ////////////////////////////////////////////////////
	...
	public static DD[] swapVars(DD[] ddArray, int[][] varMapping) {
		...
		for (int i = 0; i < ddArray.length; i++) {
			int[][] relevantVarMapping = Config.intersection(varMapping,
					ddArray[i].getVarSet());
		...
		// dd is a node
			...
			if (idx != -1) {
				...
				for (int i = 0; i < children.length; i++)
					children[i] = OP.swapVarsNoReordering(dd.getChildren()[i],
							restMapping);
	...
	public static double dotProductNoMem(DD dd1, DD dd2, SortedSet<Integer> vars) {
		...
		if (dd1.getVar() > dd2.getVar()) {
			...
			double dp = 0;
			for (int i = 0; i < dd1.getChildren().length; i++) {
				dp += OP.dotProductNoMem(dd1.getChildren()[i], dd2,
						remainingVars);
			}
			return dp;
		...
		// dd2 precedes dd1 {
			...
			double dp = 0;
			for (int i = 0; i < dd2.getChildren().length; i++) {
				dp += OP.dotProductNoMem(dd2.getChildren()[i], dd1,
						remainingVars);
			}
			return dp;
		...
		// dd2 and dd1 have same root var
			...
			double dp = 0;
			for (int i = 0; i < dd1.getChildren().length; i++) {
				dp += OP.dotProductNoMem(dd1.getChildren()[i],
						dd2.getChildren()[i], remainingVars);
			}
			return dp;
	...
	public static double dotProductLeafPrune(DD dd1, DD dd2, int[] vars) {
		...
		if (dd1.getVar() == dd2.getVar()) {
			...
			double dp = 0;
			for (int i = 0; i < dd1.getChildren().length; i++) {
				dp += OP.dotProductLeafPrune(dd1.getChildren()[i],
						dd2.getChildren()[i], remainingVars);
			}
		...
		if (dd1.getVar() > dd2.getVar()) {
			...
			double dp = 0;
			for (int i = 0; i < dd1.getChildren().length; i++) {
				dp += OP.dotProductLeafPrune(dd1.getChildren()[i], dd2,
						remainingVars);
			}
			Global.dotProductHashtable.put(triplet, new Double(dp));
			return dp;
		...
		// dd2's root precedes dd1's root
			...
			double dp = 0;
			for (int i = 0; i < dd2.getChildren().length; i++) {
				dp += OP.dotProductLeafPrune(dd1, dd2.getChildren()[i],
						remainingVars);
			}
			Global.dotProductHashtable.put(triplet, new Double(dp));
			return dp;
		 ...
		 * 
		 * // find common variable int commonVar = 0; int[] dd1VarSet =
		 * dd1.getVarSet(); int[] dd2VarSet = dd2.getVarSet(); int ptr1 =
		 * dd1VarSet.length-1; int ptr2 = dd2VarSet.length-1; while (ptr1 >= 0
		 * && ptr2 >= 0) { if (dd1VarSet[ptr1] == dd2VarSet[ptr2]) { commonVar =
		 * dd1VarSet[ptr1]; break; } else if (dd1VarSet[ptr1] > dd2VarSet[ptr2])
		 * ptr1--; else ptr2--; }
		 * 
		 * // there is a common variable if (commonVar > 0) { int[]
		 * remainingVars = MySet.remove(vars,commonVar); double dp = 0; int[][]
		 * config = new int[2][1]; config[0][0] = commonVar; for (int i=0;
		 * i<Global.varDomSize[commonVar-1]; i++) { config[1][0] = i+1; dp +=
		 * OP.
		 * dotProductLeafPrune(OP.restrictOrdered(dd1,config),OP.restrictOrdered
		 * (dd2,config),remainingVars); } return dp; }
		 * 
		 * // no common variable else { int[] remainingVars =
		 * MySet.diff(MySet.diff(vars,dd1VarSet),dd2VarSet); int
		 * multiplicativeFactor = 1; for (int j=0; j<remainingVars.length; j++)
		 * multiplicativeFactor *= Global.varDomSize[remainingVars[j]-1]; return
		 * multiplicativeFactor * dd1.getSum() * dd2.getSum(); } }
		 */
	...
	// ////////////////////////////////////////////////////
	public static double[][] dotProductNoMem(DD[] dd1Array, DD[] dd2Array,
			int[] vars) {
		double[][] results = new double[dd1Array.length][dd2Array.length];
		for (int i = 0; i < dd1Array.length; i++) {
			for (int j = 0; j < dd2Array.length; j++) {
	...
	public static double[][] factoredExpectationSparseParallel(DD[][] factDist,
			DD[] ddArray) {

		double[][] results = new double[factDist.length][ddArray.length];
		for (int i = 0; i < ddArray.length; i++) {
			double[] temp = factoredExpectationSparseParallel(factDist,
					ddArray[i]);
	...
	public static double[] factoredExpectationSparseParallelRecursive(
			DD[][] factDistArray, DD dd) {
			...
			for (int childId = 0; childId < children.length; childId++) {
				...
				for (int i = 0; i < factDistArray.length; i++) {
					if (factDistArray[i][varId] == null
							|| factDistArray[i][varId].getChildren()[childId]
									.getVal() != 0) {
						nnzIds[nnzDist] = i;
						nnzDist++;
					}
				...
				if (nnzDist > 0) {
					...
					// build array of non-zero factored distributions for each
					// child
					...
					double[] childResult = factoredExpectationSparseParallelRecursive(
							childFactDistArray, children[childId]);
					for (int id = 0; id < nnzDist; id++) {
						if (childFactDistArray[id][varId] == null)
							result[nnzIds[id]] += 1.0 / children.length
									* childResult[id];
						else
							result[nnzIds[id]] += childFactDistArray[id][varId]
									.getChildren()[childId].getVal()
									* childResult[id];
					}
				}
			}
	...
	public static double[][] factoredExpectationSparseParallel2(
			DD[][] factDist, DD[] ddArray) {

		double[][] results = new double[factDist.length][ddArray.length];
		for (int i = 0; i < ddArray.length; i++) {
			double[] temp = factoredExpectationSparseParallel2(factDist,
					ddArray[i]);
			for (int j = 0; j < temp.length; j++)
				results[j][i] = temp[j];
		}
	...
	public static double[] factoredExpectationSparseParallel2(DD[][] factDist,
			DD dd) {
		...
		return factoredExpectationSparseParallel2(factDistArray, dd,
				factDist.length, nnzIds, results);
	}

	public static double[] factoredExpectationSparseParallel2(
			DD[][][] factDistArray, DD dd, int nnzDists, int[][] nnzIds,
			double[][] results) {
			...
			for (int childId = 0; childId < children.length; childId++) {
				...
				for (int i = 0; i < nnzDists; i++) {
					if (factDistArray[varId][i][varId] == null
							|| factDistArray[varId][i][varId].getChildren()[childId]
									.getVal() != 0) {
				...
				if (nnzChildDists > 0) {
					...
					// aggregate results for each child
					double[] childResults = factoredExpectationSparseParallel2(
							factDistArray, children[childId], nnzChildDists,
							nnzIds, results);
					for (int id = 0; id < nnzChildDists; id++) {
						if (factDistArray[childVarId][id][varId] == null)
							results[varId][nnzIds[varId][id]] += 1.0
									/ children.length * childResults[id];
						else
	...
	public static double[] factoredExpectationParallel(DD[][] factDistArray,
			DD dd, HashMap hashtable) {
		...
		// it's a node
			...
			for (int childId = 0; childId < children.length; childId++) {
				...
				for (int i = 0; i < factDistArray.length; i++) {
					if (factDistArray[i][dd.getVar()] == null
							|| factDistArray[i][dd.getVar()].getChildren()[childId]
									.getVal() != 0) {
						childrenResults[childId] = OP
								.factoredExpectationParallel(factDistArray,
										children[childId], hashtable);
						break;
					}
				}
			}
			...
			for (int i = 0; i < factDistArray.length; i++) {
				if (factDistArray[i][dd.getVar()] != null) {
					DD[] scalingConstants = factDistArray[i][dd.getVar()]
							.getChildren();
					for (int childId = 0; childId < children.length; childId++) {
						result[i] += scalingConstants[childId].getVal()
								* childrenResults[childId][i];
					}
				} else {
					for (int childId = 0; childId < children.length; childId++) {
						result[i] += 1.0 / children.length
								* childrenResults[childId][i];
					}
				}
			}
		...
		// it's a node
			...
			if (varId >= 0) {
				...
				for (int i = 0; i < children.length; i++) {
					...
					result = OP.add(result, OP.addMultCutSet(restrictedDds,
							children[i], remainingVars));
				}
				return result;
			}
			...
			// root var doesn't need to be sumed out
				...
				for (int i = 0; i < children.length; i++) {
					config[1][0] = i + 1;
					DD[] restrictedDds = OP.restrictOrderedN(dds, config);
					newChildren[i] = OP.addMultCutSet(restrictedDds,
							children[i], vars);
				}
	...
	public static DD min(DD dd1, DD dd2, int[][] config) {
		...
		// the two variables have equal id
			...
			children = new DD[dd1.getChildren().length];
			for (int i = 0; i < dd1.getChildren().length; i++) {
				children[i] = OP.min(dd1.getChildren()[i],
						dd2.getChildren()[i], config);
			}
			DD result = DDnode.myNew(dd1.getVar(), children);
			Global.minHashtable.put(triplet, result);
	...
	public static boolean maxNormDiff(DD dd1, DD dd2, double threshold,
		...
		// dd2 and dd1 have same root var
			...
			for (int i = 0; i < children1.length; i++) {
				if (!OP.maxNormDiff(children1[i], children2[i], threshold,
						hashtable)) {
					result = false;
					break;
				}
	...
	public static void printPolicySpuddFormat(String filename, DD[] valuef,
			int[] pol) {
		...
		// it's a node
			...
			for (int valId = 1; valId <= children.length; valId++) {
				...
				int newIndentation = indentation
						+ Global.valNames[varId - 1][valId - 1].length() + 3;
				string += OP.displaySpuddFormat(children[valId - 1],
						newIndentation);
				string += "))";
	...
	public static void displaySpuddFormat(String fileName, DD dd,
			String indentation, int primedVarId) {
		FileOutputStream fos = null;
		PrintStream ps = null;
		try {
			fos = new FileOutputStream(fileName, true);
	...
	public static void displaySpuddFormat(PrintStream ps, DD dd,
			String indentation, int primedVarId) {
		...
		// it's a node with regular variable
			...
			for (int valId = 1; valId <= children.length; valId++) {
				...
				for (int i = 0; i < Global.valNames[varId - 1][valId - 1]
						.length() + 3; i++)
					newIndentation += " ";
				OP.displaySpuddFormat(ps, children[valId - 1], newIndentation,
						primedVarId);
				ps.print("))");
				if (valId != children.length)
					ps.print("\n" + indentation);
	...
	public static int[][] setupIP(DD dd, int[] var2row, int colId,
		...
		// it's a node
			...
			for (int i = 0; i < children.length; i++) {
				...
				nnzEntries = OP.setupIP(children[i], var2row, colId,
						nnzEntries, ptr);
	...
	public static DD approximateAll(DD dd, double tolerance) {
		...
		nLeavesPtr[0] = 0;
		nNodesPtr[0] = 0;
		return OP.approximateAll(dd, tolerance, hashtable, leafTable,
				nodeTable, nLeavesPtr, nNodesPtr, maxTable, minTable);
	}

	public static DD approximateAll(DD dd, double tolerance, HashMap hashtable,
			double[] leafTable, DD[] nodeTable, int[] nLeavesPtr,
			int[] nNodesPtr, double[] maxTable, double[] minTable) {
		...
		if (maxVal - minVal <= 2 * tolerance) {
			...
			// replace node
			if (maxVal - closestVal <= tolerance
					&& closestVal - minVal <= tolerance) {
				apprDd = DDleaf.myNew(closestVal);
				// System.out.println("nEdges(dd) = " + nEdges(dd) +
				// " nEdges(apprDd) = " + nEdges(apprDd));
			}
		...
		// check if dd can be approximated by a node
			...
			int id = 0;
			// while (id<nNodesPtr[0] && dd.getNumLeaves() >=
			// nodeTable[id].getNumLeaves()) {
			while (id < nNodesPtr[0] && dd.getVar() >= nodeTable[id].getVar()) {

				// nodeTable[id] approximates dd
				if (maxTable[id] - maxVal <= tolerance
						&& maxVal - maxTable[id] <= tolerance
						&& minTable[id] - minVal <= tolerance
						&& minVal - minTable[id] <= tolerance
			...
			// no existing node approximates dd so try to approximate children
			// recursively
			DD[] children = dd.getChildren();
			DD[] newChildren = new DD[children.length];
			for (int i = 0; i < children.length; i++) {
				newChildren[i] = OP.approximateAll(children[i], tolerance,
						hashtable, leafTable, nodeTable, nLeavesPtr, nNodesPtr,
						maxTable, minTable);
			}
	...
	public static DD approximate(DD dd, double tolerance, HashMap hashtable,
			double[] leafValues, int[] nLeavesPtr) {
